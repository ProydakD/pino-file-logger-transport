...
- **Умная ротация логов** - Автоматическая ротация по дате и размеру с настраиваемым именованием
...
- Опции ротации по размеру (`maxFileSizeMB`, `maxFiles`) доступны начиная с версии `2.1.0`.
...
const transport = pino.transport({
  ...
  options: {
    ...
    // Опционально: Ротация по размеру файла в MB (по умолчанию: отключено, с 2.1.0)
    maxFileSizeMB: 50,

    // Опционально: Максимум управляемых файлов на директорию (по умолчанию: отключено, с 2.1.0)
    maxFiles: 100,
  },
});
