...
describe('File Transport', () => {
  ...
  it('should filter logs when level is passed as string label', async () => {
    const filename = 'level-string-test';
    const transport = fileTransport({
      logDirectory: testDir,
      filename,
      level: 'error',
    ...
    transport.write(JSON.stringify({ level: 'warn', msg: 'warn message' }) + '\n');
    transport.write(JSON.stringify({ level: 'error', msg: 'error message' }) + '\n');
    transport.write(JSON.stringify({ level: '60', msg: 'fatal message' }) + '\n');
    ...
    const date = new Date();
    const dateString = `${date.getFullYear()}-${String(date.getMonth() + 1).padStart(2, '0')}-${String(date.getDate()).padStart(2, '0')}`;
    const logFile = join(testDir, `${filename}-${dateString}.log`);
    const logContent = readFileSync(logFile, 'utf8');
    const lines = logContent.trim().split('\n');

    expect(lines).toHaveLength(2);
    const logEntries = lines.map((line) => JSON.parse(line));
    expect(logEntries.some((entry) => entry.msg === 'warn message')).toBe(false);
    expect(logEntries.some((entry) => entry.msg === 'error message')).toBe(true);
    expect(logEntries.some((entry) => entry.msg === 'fatal message')).toBe(true);
  });

  it('should filter logs when level is nested in payload', async () => {
    const filename = 'level-nested-test';
    const transport = fileTransport({
      logDirectory: testDir,
      filename,
      level: 'warn',
    ...
    transport.write(
      JSON.stringify({
        log: { level: { value: 30 } },
        msg: 'nested info message',
      }) + '\n',
    );
    transport.write(
      JSON.stringify({
        record: { level: { label: 'error' } },
    ...
    expect(logEntry.msg).toBe('nested error message');
  });

  it('should process logs generated by pino logger', async () => {
    const filename = 'pino-integration-test';
    const transport = fileTransport({
      logDirectory: testDir,
      filename,
      level: 'warn',
    ...
    const logger = pino({ level: 'trace' }, bridge);
